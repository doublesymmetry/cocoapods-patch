...
cocoapods-patch is a Cocoapods plugin that solves the problem of forking and maintaining a separate version of a Pod only because a small (often a one-liner), long-lived change in the original Pod is needed.
...
Next, add a `plugin 'cocoapods-patch'` line to your Podfile to use the plugin.
